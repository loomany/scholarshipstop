...
export type MoreFiltersState = {
  ...
  /** Include-only: scholarship must match ≥1 selected requirement type (OR). */
...
function matchesRequiredType(s: Scholarship, typeId: string): boolean {
...
export function scholarshipPassesMoreFilters(
  ...
  const requiredTypes = Array.from(f.includeRequirementTypes);
  if (requiredTypes.length > 0) {
    let hasRequiredType = false;
    for (let i = 0; i < requiredTypes.length; i++) {
      if (matchesRequiredType(s, requiredTypes[i]!)) {
