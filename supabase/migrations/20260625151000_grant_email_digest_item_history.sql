...
-- Per-user email digest item history.
-- Used to rotate fallback recommendations without repeating the same grants every day.
...
create table if not exists public.grant_email_digest_items (
  ...
  section text not null check (
    section in ('best', 'saved_filters', 'easy_apply', 'hot_deadlines', 'recommended')
  ),
  digest_kind text not null default 'daily_grant_digest',
  created_at timestamptz not null default now()
);
...
create index if not exists grant_email_digest_items_user_scholarship_idx
  on public.grant_email_digest_items (user_id, scholarship_id);
