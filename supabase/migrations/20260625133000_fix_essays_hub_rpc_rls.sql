-- Keep /essays public for anon after locking down direct reads on
-- public.scholarships. The RPC only needs public category fields for the first
-- linked scholarship, so read them through scholarships_safe_listing instead
-- of the raw base table.
...
as $$
  ...
  base as (
    ...
    left join lateral (
      ...
      inner join public.scholarships_safe_listing s on s.id = se.scholarship_id
...
comment on function public.essays_hub_index_page(text, text, text, int, int) is
  'Paginated /essays hub using scholarships_safe_listing for public linked-scholarship category lookups.';
