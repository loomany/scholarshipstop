...
export type SeoTelegramNotifyInput = {
  title: string;
  /** Extra line after title (optional). */
  message?: string;
...
/** Count how many entries mention each issue/warning code (deduped per entry). */
...
/**
 * Sends one Telegram message. Never throws — failures return { sent: false, reason }.
 * Does not log the full bot token (only a short prefix on errors).
 */
