...
/**
 * Where automated resource posts go: TELEGRAM_RESOURCES_CHAT_ID first,
 * else every admin Telegram chat with notifications on.
 */
...
export async function sendTelegramResourceNotification(
  ...
  if (chatIds.length === 0) {
    console.error(
      '[telegram-resources] no target chats (set TELEGRAM_RESOURCES_CHAT_ID or link admins in telegram_users)'
    );
    return false;
  }
