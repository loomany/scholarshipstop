...
function resolveModel(modelFromEnv) {
...
function logAiCall(taskName, model) {
    logger.info(`[AI] Calling ${model} for ${taskName}...`);
}
function logOpenAiFailure(taskName, model, error) {
...
async function generateJson(prompt, taskName, model) {
    const raw = await chatCompletionText({
        ...
        messages: [
            {
                role: "system",
                content: "You output only a single valid JSON object. No markdown code fences, no commentary before or after the JSON."
            },
...
const HEADING_NORMALIZATION_RULES = [
    {
...
function validateArticleStructure(article) {
    ...
    const actionSection = sectionRanges.find((section) => /step|how to|action plan|plan/i.test(section.title) || /Step-by-Step Action Plan/i.test(section.normalizedTitle));
...
function splitIntro(bodyMarkdown) {
...
async function autoFixMissingH1(article) {
...
export async function generateSeoBrief(topic, promptOverlay = "") {
...
export async function generateArticle(topic, seoBrief, context, options, promptOverlay = "") {
    ...
    const retryRaw = await generateJson(buildArticleExpansionPrompt(topic, seoBrief, context, firstArticle, retryIssues, options), "article_expansion_retry", smartModel);
    ...
    if (retryWordCount <= wordCount) {
        ...
        const aggressiveRetryRaw = await generateJson(buildArticleExpansionPrompt(topic, seoBrief, context, retryArticle, aggressiveRetryIssues, options, "aggressive_depth"), "article_expansion_aggressive_retry", smartModel);
...
export async function expandShortArticleBody(bodyMarkdown, currentCount, minWords) {
...
export async function addExternalAuthorityLinksToHtml(bodyHtml) {
...
export async function autoFixArticleByMetrics(bodyMarkdown, metricsErrors) {
