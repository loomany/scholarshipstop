...
export function parseAiPackTopicString(raw) {
    if (!raw.startsWith(AI_PACK_PREFIX))
        return null;
    const map = new Map();
...
export function assertProductionWritesAllowed() {
    if (process.env.CONTENT_HUB_ALLOW_PRODUCTION_WRITES !== "1") {
        throw new Error("Refusing DB write: set CONTENT_HUB_ALLOW_PRODUCTION_WRITES=1 explicitly for production Content Hub writes.");
    }
}
export function resolveExpectedPackSource() {
    const v = process.env.CONTENT_HUB_SOURCE?.trim();
    return v || null;
}
export function topicMatchesPackSource(topicText) {
...
export function buildAiPackPromptOverlay(parsed) {
    return [
