const ARTICLE_TYPES = ["list", "guide", "strategy", "comparison", "niche", "deep_dive"];
const INTRO_STYLES = ["problem", "question", "statistic", "story", "direct"];
const IMAGE_STYLES = ["realistic", "editorial"];
const SCENE_LIBRARY = [
    {
        id: "people",
        promptTokens: [
...
export function pickArticleType(topic) {
    const normalized = topic.toLowerCase();
    if (/top\b|best\b|list\b/.test(normalized))
        return "list";
    if (/how to\b|step by step\b|apply\b/.test(normalized))
        return "guide";
    if (/tips?\b|mistakes?\b|strategy\b/.test(normalized))
        return "strategy";
    if (/\bvs\b|versus\b|compare|comparison/.test(normalized))
        return "comparison";
    if (/for women|for low income|for international|for first generation|for veterans/.test(normalized))
        return "niche";
    if (/what is\b|how .* works|explained/.test(normalized))
        return "deep_dive";
    return weightedPick([
...
export function pickIntroStyle() {
    return INTRO_STYLES[Math.floor(Math.random() * INTRO_STYLES.length)];
}
export function shuffleSections(sections) {
    const copy = [...sections];
...
export function pickImageStyle(topic, articleType) {
    void topic;
    void articleType;
    return "realistic";
}
...
export function pickCompositionVariant(scene) {
...
export function isImagePromptTooSimilar(nextPrompt, lastPrompts) {
...
export function pickAlternativeImageStyle(current) {
