...
export function extractImageUrlFromFalJson(data, depth = 0) {
    if (depth > 10 || !data || typeof data !== "object")
        return null;
    const d = data;
    const asHttpUrl = (u) => typeof u === "string" && u.startsWith("http") ? u : null;
    if (asHttpUrl(d.url))
        return asHttpUrl(d.url);
    const maybeSingle = d.output && typeof d.output === "object" && !Array.isArray(d.output)
        ? d.output
        : d.image && typeof d.image === "object" && !Array.isArray(d.image)
            ? d.image
            : null;
    if (maybeSingle) {
        const u = asHttpUrl(maybeSingle.url);
        if (u)
            return u;
    }
    ...
    for (const nestKey of ["data", "result", "response", "output"]) {
