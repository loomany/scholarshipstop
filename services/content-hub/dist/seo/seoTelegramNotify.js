...
export function isSeoTelegramNotifyConfigured() {
...
export function isSeoTelegramNotifyEnabled() {
    return process.env.SEO_TELEGRAM_NOTIFY_ENABLED?.trim() === "1";
}
function tokenPrefixForLog() {
    const t = process.env.SEO_TELEGRAM_BOT_TOKEN?.trim();
    if (!t)
        return "(unset)";
    return `${t.slice(0, 6)}…`;
}
export function listSeoIssueCountsSorted(entries) {
    const map = new Map();
    for (const e of entries) {
        const codes = new Set([...e.issues, ...e.warnings]
            .map((c) => c.trim())
            .filter(Boolean));
...
export function buildSeoGuardTelegramHtml(input) {
    ...
    examples.forEach((e, i) => {
        lines.push(`${i + 1}. <code>${escapeTelegramHtml(e.url)}</code> — score ${e.score}`);
    });
    if (sorted.length > 5) {
...
export async function sendSeoTelegramNotification(input) {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            console.error("[seo-telegram] sendMessage failed", res.status, "token=", tokenPrefixForLog(), body.slice(0, 200));
            return { sent: false, reason: `http_${res.status}` };
        }
        return { sent: true };
    }
    catch (e) {
        console.error("[seo-telegram] sendMessage error", e instanceof Error ? e.message : String(e), "token=", tokenPrefixForLog());
