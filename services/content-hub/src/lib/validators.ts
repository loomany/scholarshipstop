...
import type { ArticleType, IntroStyle } from "./variation.js";
...
function normalizeImageBriefField(val: unknown): unknown {
...
interface ArticleMetricsThresholds {
  minTotalWords: number;
  minCharsNoSpaces: number;
...
interface ValidationCheckFailure {
  check: string;
  expected: string;
...
interface ArticleMetricsSnapshot {
  ...
  detectedIntroStyle: IntroStyle;
  bodyHtmlLength: number;
  textLength: number;
  charCountNoSpaces: number;
}

export interface ArticleMetricsValidationContext {
  topic: string;
  articleType: ArticleType;
  introStyle: IntroStyle;
}

export interface ArticleMetricsValidationResult {
  ok: boolean;
  hardFailed: boolean;
  failedChecks: string[];
  softWarnings: string[];
  metrics: ArticleMetricsSnapshot;
  thresholds: ArticleMetricsThresholds;
  wordCount: number;
  charCount: number;
}

const THRESHOLDS_BY_ARTICLE_TYPE: Record<ArticleType, ArticleMetricsThresholds> = {
  list: {
...
function detectIntroStyle(text: string): IntroStyle {
  const normalized = text.trim().toLowerCase();
  if (/^\s*["'“][^"'”]+["'”]/.test(normalized)) return "story";
  if (/\?/.test(normalized.split("\n")[0] || "")) return "question";
  if (/\d+%|\d+\s*(students|applicants|scholarships|awards)/.test(normalized)) return "statistic";
  if (/problem|challenge|struggle|hard|difficult/.test(normalized)) return "problem";
  return "direct";
}

function buildFailure(check: string, expected: string, actual: number | string | boolean, severity: ValidationSeverity): ValidationCheckFailure {
  return {
    check,
    expected,
    actual,
...
async function persistValidationDebugPayload(payload: Record<string, unknown>, slug: string): Promise<void> {
  try {
...
export async function validateArticleMetrics(
  article: GeneratedArticle,
  context: ArticleMetricsValidationContext
): Promise<ArticleMetricsValidationResult> {
  ...
  const failed: ValidationCheckFailure[] = [];
  ...
  const metrics: ArticleMetricsSnapshot = {
  ...
  const result: ArticleMetricsValidationResult = {
