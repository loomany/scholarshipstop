...
const FAL_FLUX_DEV_MODEL_ID = "fal-ai/flux/dev" as const;
...
type FalAttemptResult = { url: string | null; httpStatus: number; detail: string };

function falMaxAttempts(): number {
...
function falSingleRequestTimeoutMs(): number {
  const hero = env.FAL_HERO_REQUEST_TIMEOUT_MS;
  if (hero !== undefined && hero >= 30_000) return Math.min(hero, 300_000);
  const n = env.FAL_REQUEST_TIMEOUT_MS;
  return Math.min(Math.max(n, 30_000), 300_000);
}

function buildFluxDevImageBody(prompt: string): Record<string, unknown> {
...
function shouldRetryFalAttempt(attemptIndex: number, maxAttempts: number, r: FalAttemptResult): boolean {
...
async function postFluxDevImageOnce(fullImagePrompt: string): Promise<FalAttemptResult> {
  ...
  const raw =
    json && typeof json === "object" ? JSON.stringify(json).slice(0, 900) : String(json);
...
async function runFalImageAttemptLoop(
  attemptOne: () => Promise<FalAttemptResult>,
  logPrefix: string
): Promise<FalAttemptResult> {
  const maxAttempts = falMaxAttempts();
  let last: FalAttemptResult = { url: null, httpStatus: 0, detail: "no attempts" };
