...
function isStaleProcessing(topic: TopicRow): boolean {
  return (
...
async function main() {
  const [topics, posts] = await Promise.all([
    fetchAll<TopicRow>("content_topics", "id,topic,status,updated_at"),
    fetchAll<PostRow>("content_posts", "topic_id")
  ]);
  const postTopicIds = new Set(posts.map((post) => post.topic_id));
  const candidates = topics.filter((topic) => {
    if (includeFailed && topic.status === "failed" && !postTopicIds.has(topic.id)) return true;
    if (includeProcessing && isStaleProcessing(topic)) return true;
    if (includeDoneWithoutPost && topic.status === "done" && !postTopicIds.has(topic.id)) return true;
    return false;
  });
