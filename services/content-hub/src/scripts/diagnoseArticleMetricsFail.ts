...
function parseFailedCheck(entry: string): { metric: string; expected: string; actual: string } | null {
  const match = entry.match(/^([^:]+):\s*expected\s+(.+?),\s*got\s+(.+)$/i);
  if (!match) return null;
  return { metric: match[1].trim(), expected: match[2].trim(), actual: match[3].trim() };
}

function isGeneralTopic(topic: string): boolean {
...
function classifyFailure(topic: string, failedChecks: string[]): string {
  const parsed = failedChecks.map(parseFailedCheck).filter((x): x is NonNullable<typeof x> => Boolean(x));
  ...
  if (isGeneralTopic(topic) && depth) return "too broad (generic topic scope led to shallow depth)";
  if (depth && !structural) return "too thin (insufficient content depth)";
  if (structural) return "structurally weak (insufficient sections/paragraph density)";
  return "mixed quality issue";
}

async function findLatestPayloadFile(): Promise<string | null> {
...
async function main() {
  ...
  const payload = JSON.parse(raw) as ValidationPayload;
  const topic = payload.topic ?? "unknown";
  const failedChecks = payload.failedChecks ?? [];
  const parsedFailed = failedChecks.map(parseFailedCheck).filter((item): item is NonNullable<typeof item> => Boolean(item));
  const diagnosis = classifyFailure(topic, failedChecks);

  let queuedTopics: string[] = [];
  try {
