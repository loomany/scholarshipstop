...
type PostRow = {
  ...
  metrics_debug: string | null;
};

async function fetchAll<T>(table: string, select: string, orderColumn = "created_at"): Promise<T[]> {
  const out: T[] = [];
  const pageSize = 1000;
...
function isOlderThanHours(dateValue: string | null | undefined, hours: number): boolean {
...
async function main() {
  const [topics, posts] = await Promise.all([
    fetchAll<TopicRow>(
      "content_topics",
      "id,topic,status,updated_at,attempt_count,failure_class,last_stage,last_error"
    ),
    fetchAll<PostRow>(
      "content_posts",
      "id,topic_id,title,slug,status,published_at,cover_image_url,cover_image_source_url,word_count,metrics_debug"
    )
  ]);

  const postTopicIds = new Set(posts.map((post) => post.topic_id));
  const staleProcessing = topics.filter(
    (topic) => topic.status === "processing" && isOlderThanHours(topic.updated_at, env.CONTENT_HUB_STALE_PROCESSING_HOURS)
  );
    ...
    JSON.stringify(
      {
        ...
        topic_failure_class_counts: countBy(
          topics.filter((topic) => topic.status === "failed" || topic.failure_class),
          (topic) => topic.failure_class
        ),
        ...
        published_without_source_tracking: published.filter(
          (post) => post.cover_image_url?.trim() && !post.cover_image_source_url?.trim()
        ).length,
