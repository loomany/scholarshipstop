...
import {
  enforceAllowedScholarshipBodyLinks,
  findScholarshipsForIntent,
  injectInternalLinks,
...
import {
  isImagePromptTooSimilar,
  pickCompositionVariant,
  pickAlternativeImageStyle,
  pickArticleType,
  pickImageStyle,
  pickIntroStyle,
  selectScene
...
type JobStage =
  ...
  | "verifying_scholarship_links"
  | "matching_related_articles"
  | "verifying_related_links"
  | "validating_metrics"
  | "generating_image"
  | "processing_image"
  | "uploading_image"
...
type MetricsStrictness = "strict" | "medium" | "relaxed";
...
function logUnknownError(message: string, error: unknown, data?: Record<string, unknown>) {
  const normalized = normalizeError(error);
  logger.error(message, { ...data, error: normalized.message, stack: normalized.stack });
}

function isTransientPipelineError(error: unknown): boolean {
  const normalized = normalizeError(error);
...
function shouldRequireCoverImageForAutoPublish(): boolean {
  return env.CONTENT_HUB_AUTO_PUBLISH === 1 && env.CONTENT_HUB_REQUIRE_COVER_FOR_PUBLISH === 1;
}

function pickGeneratedLinks(
  links: { title: string; url: string }[],
  max: number,
  category: "scholarship" | "faq"
) {
...
function resolveLinkingFlag(primary: number, aliases: string[]): boolean {
...
function countExternalAuthorityLinks(body: string): number {
...
function countInternalScholarshipLinks(bodyHtml: string): number {
...
function ensureTldrBlockAtEnd(
  bodyHtml: string,
  context: { title: string; h1: string; excerpt?: string; metaDescription?: string }
): string {
  const existing = /<div\b[^>]*class=["'][^"']*\barticle-tldr-block\b[^"']*["'][^>]*>[\s\S]*?<\/div>\s*$/i;
  if (existing.test(bodyHtml.trim())) return bodyHtml;
...
function appendFallbackSourcesBlock(bodyHtml: string): string {
  if (/<h2[^>]*>\s*Sources\s*<\/h2>/i.test(bodyHtml)) return bodyHtml;
  const sourcesBlock = `
<h2>Sources</h2>
<ul>
...
function appendFallbackSourcesMarkdown(bodyMarkdown: string): string {
  if (/^##\s+Sources\s*$/im.test(bodyMarkdown)) return bodyMarkdown;
...
function isAllowedExternalDomain(hostname: string): boolean {
  ...
  if (EXTERNAL_REFERENCE_ALLOWED_EXACT.has(host)) return true;
...
function isBlockedCompetitorDomain(hostname: string): boolean {
  const host = hostname.toLowerCase();
  if (EXTERNAL_COMPETITOR_BLOCKLIST.has(host)) return true;
...
function sanitizeExternalAuthorityLinks(bodyHtml: string): { html: string; removedCount: number; normalizedCount: number } {
  let removedCount = 0;
...
function isCriticalHtmlIssue(updatedHtml: string, originalHtml: string): boolean {
  if (!updatedHtml.trim()) return true;
  if (!/<h1\b/i.test(updatedHtml) && /<h1\b/i.test(originalHtml)) return true;
  if (updatedHtml.length < Math.floor(originalHtml.length * 0.6)) return true;
  return false;
}

type MaintenanceArticle = {
  id: string;
  slug: string;
  body_html: string;
  scholarship_links: Array<{ slug: string; title: string; reason: string; url?: string }>;
  related_article_links: Array<{ slug: string; title: string; reason: string; url?: string }>;
};
...
async function runStartupReprocess(): Promise<void> {
  ...
  const articles = (data ?? []).map((row) => ({
    ...
    scholarship_links: normalizeJsonArray<{ slug: string; title: string; reason: string; url?: string }>(row.scholarship_links),
    related_article_links: normalizeJsonArray<{ slug: string; title: string; reason: string; url?: string }>(row.related_article_links)
  })) as MaintenanceArticle[];
...
const lastImagePrompts: string[] = [];
const lastImageScenes: Array<ReturnType<typeof selectScene>["id"]> = [];
...
function getSafePublicationIntervalMinutes(): number {
...
async function processOneTopic(): Promise<"published" | "deferred" | "empty"> {
  const minWords = getEffectiveMinWords();
  let stage: JobStage = "picking_topic";
  ...
  try {
    ...
    let anchorSuggestions = ((article as { anchor_suggestions?: AnchorSuggestion[] }).anchor_suggestions ?? []);
    let intentMatches: Array<{ anchorText: string; intent: string; scholarship: { slug: string; title: string } }> = [];
    if (!scholarshipLinkingEnabled) {
      ...
      try {
        if (anchorSuggestions.length === 0) {
          ...
          const intentMatchesNested = await Promise.all(
            anchorSuggestions.map(async (suggestion) => {
      ...
      article.scholarship_links = intentMatches
        ...
        .filter((entry): entry is { slug: string; title: string } => Boolean(entry?.slug) && Boolean(entry?.title))
    ...
    if (!relatedArticlesEnabled) {
      ...
      try {
        article.related_article_links = pickRelatedArticles(
          publishedArticles,
          article.slug,
          env.CONTENT_HUB_MIN_RELATED_ARTICLES,
          env.CONTENT_HUB_MAX_RELATED_ARTICLES
        );
    ...
    let { html: processedContent, removedCount: removedExternalCount, normalizedCount: normalizedExternalCount } =
      sanitizeExternalAuthorityLinks(scholarshipSafeContent);
    ...
    const strictness = env.METRICS_STRICTNESS as MetricsStrictness;
    ...
    let status: "published" | "review_needed";
    if (strictness === "relaxed") {
    ...
    if (process.env.DEBUG_MODE === "true") {
      ...
      console.log(
        `[DEBUG_1_FLAGS] PROCESS_EXTERNAL_LINKS=${env.PROCESS_EXTERNAL_LINKS}, shouldAppendFallback=${shouldAppendFallback}`
      );
      console.log(
        `[DEBUG_2_SANITIZER] externalLinks before sanitize=${externalLinksBeforeSanitize}, after sanitize=${externalLinksAfterSanitize}`
      );
...
async function main() {
  ...
  if (env.CONTINUOUS_MODE) {
    while (true) {
      let result: "published" | "deferred" | "empty" = "deferred";
      try {
