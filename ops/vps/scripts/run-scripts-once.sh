#!/usr/bin/env bash
# One-shot scripts cron (enrich + essay pipeline). Never prints env values.
set -euo pipefail
ROOT=/opt/scholarshiptop
APP="${ROOT}/app"
ENV_FILE="${ROOT}/env/scripts.env"
LOG_DIR="${ROOT}/logs"
TS="$(date -u +%Y%m%dT%H%M%SZ)"
LOG_FILE="${LOG_DIR}/scripts-run-${TS}.log"
...
(
  ...
  node --env-file="${ENV_FILE}" -e "
    ...
    const r = spawnSync('bash', ['scripts/railway-cron.sh', 'all'], {
      cwd: process.cwd(),
      env,
      stdio: 'inherit'
    });
