/**
 * Stage 5E-15 wave-size 150 baseline before wave 161+ (expect 7701/7701).
 */
...
import { auditTieredPool } from './select-candidates-tiered';
...
async function main() {
  ...
  if (
    ...
    issues.push('bad sitemap content');
  }
  ...
  for (let i = 0; i < 10; i++) {
    const slug = `unseeded-5e15-check-${i}-xyz`;
  ...
  const body = `# Stage 5E-15 wave-size 150 baseline (${DATE})
...
| Tier A | ${audit.tierCounts.A} |
| Tier B | ${audit.tierCounts.B} |
| Publishable A+B | ${audit.publishable.length} |

## Verdict: **${passed ? 'PASS' : 'FAIL'}**

${issues.length ? issues.map((i) => `- ${i}`).join('\n') : '- no issues'}
`;
  const path = join(process.cwd(), 'reports/seo', `i18n-stage5e-15-wave150-baseline-${DATE}.md`);
    ...
    JSON.stringify(
      {
        ...
        liveFr,
        distinct,
        publishable: audit.publishable.length
      },
      null,
      2
    )
