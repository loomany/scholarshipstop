/**
 * Next scholarship_detail candidates (excludes current 56 pilots).
 */
...
async function main() {
  ...
  const csvPath = join(
    process.cwd(),
    'reports/seo',
    `i18n-stage5e-scholarship-detail-next-candidates-${DATE}.csv`
  );
  writeFileSync(csvPath, header + lines.join('\n') + '\n', 'utf8');
  console.log(`Wrote ${csvPath} (${picked.length} candidates, batches 6-${batch})`);
}
